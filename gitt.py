...
wr = do + "(" + clightv + "!" + do + ")"
kt1 = f"[#00FF00][[#00FFFF]⊀[#FFFFFF]/[#00FFFF]⊁[#00FF00]][#FFFFFF]=[bold #00C8C8]> "
#==≠==≠=Admin=≠==≠==#
...
def ma_defile(headers, username, repo):
    ...
    if r.status_code == 200:
        ...
        if not items:
            console.print("Repo không có file nào để xoá.", style="bold red")
            return
        minhanhs()
        table = Table(title="bold Danh sách File", border_style="#7FFF00", show_header=True, header_style="bold cyan", box=box.HEAVY)
        table.add_column("STT", justify="center", style="bold green")
        table.add_column("File name", justify="left", style="yellow")

        for idx, item in enumerate(items, 1):
            table.add_row(str(idx), item["name"])
...
def main():
    ...
    while True:
        table = Table(show_header=True, header_style="bold cyan", box=box.HEAVY_EDGE)
        table.add_column("Lựa chọn", justify="center", style="bold green")
        table.add_column("Chức năng", justify="center", style="bold yellow")

        table.add_row("1", "Upload - Tạo file mới")
        table.add_row("2", "Đổi repo")
        table.add_row("3", "Xoá file")
        table.add_row("4", "Xoá repo")
        table.add_row("5", "Thoát")

        console.print(kt1 + f"Repo hiện tại: {repo}")
        console.print(table)
